...
export default async function saasBillingRoutes(app: FastifyInstance) {
  ...
  app.post('/checkout', {
    ...
    // Calculate next due date (today + 3 days for first payment)
    ...
    try {
      // 4. Find or create Asaas customer (no charge generated by this step)
      ...
      // 5. Create tenant in TRIAL status FIRST — fails fast on subdomain conflicts
      //    or schema/validation errors BEFORE the Asaas subscription is created.
      //    This prevents orphan billing where the user gets charged but no tenant exists.
      const tenant = await prisma.tenant.create({
        data: {
          ...
          settings: {
            ...
            customerPhone: body.customer.phone || body.customer.mobilePhone || null,
          },
        },
      })
      tenantId = tenant.id

      // 6. Create Asaas subscription (this is what triggers the billing email)
      ...
      // 7. Persist subscription id on tenant
      ...
      // Structured logging so we can see the real cause in Railway logs.
      app.log.error(
        { err, stack: err?.stack, code: err?.code, meta: err?.meta, subdomain: body.subdomain, planSlug: body.planSlug },
        '[saas-billing] Checkout failed',
      )

      // Rollback: if the Asaas subscription was created but persistence failed,
      // cancel the Asaas subscription so the user is not billed for nothing.
      if (asaasSubscriptionId) {
        await cancelSubscription(asaasSubscriptionId).catch((cancelErr: any) => {
          app.log.error(
            { err: cancelErr, asaasSubscriptionId, tenantId },
            '[saas-billing] Failed to cancel orphan Asaas subscription — manual cleanup required',
          )
        })
      }

      // Rollback: remove the orphan tenant if it was created.
      if (tenantId) {
      ...
      await app.prisma.auditLog.create({
        data: {
          ...
          payload: {
            ...
            asaasSubscriptionId,
            tenantId,
            rolledBack: !!(asaasSubscriptionId || tenantId),
          } as any,
        },
      }).catch(() => {})
