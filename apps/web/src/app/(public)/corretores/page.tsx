...
function MembroCard({ membro }: { membro: Membro }) {
  ...
  return (
    <div className="bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 border border-stone-100 flex flex-col">
      {/* Header gradiente */}
      <div className="relative h-48 flex items-end justify-center"
        ...
        <div className="relative translate-y-1/2 z-10">
          <div className="w-28 h-28 rounded-full overflow-hidden border-4 shadow-xl bg-amber-400 flex items-center justify-center"
            ...
            <img
              ...
              onError={(e) => {
                ...
                if (parent) {
                  parent.innerHTML = `<span style="color:#1B2B5B;font-size:2.5rem;font-weight:bold;line-height:1">${initial}</span>`
                }
              }}
            />
          </div>
      ...
      {/* Conteúdo */}
      <div className="pt-16 pb-6 px-6 text-center flex flex-col flex-1">
        <h3 className="text-lg font-bold" style={{ color: '#1B2B5B', fontFamily: 'Georgia, serif' }}>
          {membro.name}
        </h3>
        <p className="text-sm text-gray-500 mt-0.5">{membro.role}</p>
        {membro.creci && (
        ...
        {membro.bio && (
          <p className="text-sm text-gray-600 mt-3 leading-relaxed flex-1">{membro.bio}</p>
        )}
        ...
        <a
          ...
          className="mt-5 flex items-center justify-center gap-2 w-full py-3 rounded-2xl font-semibold text-white transition-all hover:opacity-90 active:scale-95"
