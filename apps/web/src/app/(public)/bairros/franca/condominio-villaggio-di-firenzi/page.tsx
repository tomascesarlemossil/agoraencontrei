...
export const metadata: Metadata = {
  title: 'Imóveis no CONDOMINIO VILLAGGIO DI FIRENZI, Franca/SP | Imobiliária Lemos — CRECI 279051',
  description: 'Casas à venda e para alugar no CONDOMINIO VILLAGGIO DI FIRENZI em Franca/SP. Imobiliária Lemos — 22 anos de tradição. CRECI 279051. Encontre o imóvel ideal no CONDOMINIO VILLAGGIO DI FIRENZI com atendimento personalizado.',
  keywords: [
    'imóveis condominio villaggio di firenzi franca sp', 'casas condominio villaggio di firenzi franca', 'apartamentos condominio villaggio di firenzi franca',
    'comprar casa condominio villaggio di firenzi franca sp', 'alugar condominio villaggio di firenzi franca sp',
    'terrenos condominio villaggio di firenzi franca', 'imóveis franca sp condominio villaggio di firenzi',
    'imobiliária lemos condominio villaggio di firenzi', 'condominio villaggio di firenzi franca sp imóveis',
    'casas à venda condominio villaggio di firenzi', 'apartamentos aluguel condominio villaggio di firenzi',
  ],
  openGraph: {
    title: 'Imóveis no CONDOMINIO VILLAGGIO DI FIRENZI, Franca/SP | Imobiliária Lemos',
    description: 'Encontre casas, apartamentos e terrenos no CONDOMINIO VILLAGGIO DI FIRENZI em Franca/SP com a Imobiliária Lemos.',
    type: 'website', locale: 'pt_BR', siteName: 'AgoraEncontrei — Imobiliária Lemos',
  },
  alternates: { canonical: 'https://www.agoraencontrei.com.br/bairros/franca/condominio-villaggio-di-firenzi' },
...
const SCHEMA = {
  ...
  name: 'Imóveis no CONDOMINIO VILLAGGIO DI FIRENZI, Franca/SP',
  description: 'Imóveis disponíveis no CONDOMINIO VILLAGGIO DI FIRENZI em Franca, São Paulo. Imobiliária Lemos — CRECI 279051.',
  url: 'https://www.agoraencontrei.com.br/bairros/franca/condominio-villaggio-di-firenzi',
  provider: {
...
async function fetchProperties() {
  try {
    const res = await fetch(
      `${API_URL}/api/v1/public/properties?city=Franca&neighborhood=CONDOMINIO VILLAGGIO DI FIRENZI=${encodeURIComponent('CONDOMINIO VILLAGGIO DI FIRENZI')}&limit=12&status=ACTIVE&sortBy=createdAt&sortOrder=desc`,
      { next: { revalidate: 600 } }
    )
      ...
      <section className="bg-gradient-to-br from-[#1B2B5B] to-[#0f1c3a] text-white py-14 px-4">
        <div className="max-w-5xl mx-auto">
          <nav className="text-xs text-blue-200 mb-4 flex items-center gap-1.5 flex-wrap">
            ...
            <span>/</span>
            <span className="text-white">CONDOMINIO VILLAGGIO DI FIRENZI</span>
          </nav>
          <h1 className="text-3xl sm:text-4xl font-bold mb-3" style={{ fontFamily: 'Georgia, serif' }}>
            Imóveis no <span style={{ color: '#C9A84C' }}>CONDOMINIO VILLAGGIO DI FIRENZI</span>
            <br /><span className="text-xl font-normal text-white/70">Franca/SP</span>
          </h1>
          <p className="text-white/70 text-base mb-5 max-w-2xl">
            {total > 0 ? `${total} imóveis disponíveis` : 'Imóveis disponíveis'} no CONDOMINIO VILLAGGIO DI FIRENZI, Franca/SP.
            Casas, apartamentos, terrenos e comerciais. Imobiliária Lemos — CRECI 279051.
          </p>
          <div className="flex flex-wrap gap-2">
            <Link href={`/imoveis?city=Franca&neighborhood=CONDOMINIO VILLAGGIO DI FIRENZI&type=HOUSE`}
            ...
            <Link href={`/imoveis?city=Franca&neighborhood=CONDOMINIO VILLAGGIO DI FIRENZI&type=APARTMENT`}
              className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-full text-sm font-medium border border-white/20 transition-colors">
              Apartamentos
            </Link>
            <Link href={`/imoveis?city=Franca&neighborhood=CONDOMINIO VILLAGGIO DI FIRENZI&type=LAND`}
              className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-full text-sm font-medium border border-white/20 transition-colors">
              Terrenos
            </Link>
            <Link href={`/imoveis?city=Franca&neighborhood=CONDOMINIO VILLAGGIO DI FIRENZI&purpose=RENT`}
              className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-full text-sm font-medium border border-white/20 transition-colors">
              Para Alugar
            </Link>
            <a href="https://wa.me/5516981010004?text=Olá! Quero um imóvel no CONDOMINIO VILLAGGIO DI FIRENZI em Franca/SP."
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold" style={{ color: '#1B2B5B', fontFamily: 'Georgia, serif' }}>
            {total > 0 ? `${total} Imóveis no CONDOMINIO VILLAGGIO DI FIRENZI` : 'Imóveis no CONDOMINIO VILLAGGIO DI FIRENZI'}
          </h2>
          <Link href={`/imoveis?city=Franca&neighborhood=CONDOMINIO VILLAGGIO DI FIRENZI`}
        ...
        {properties.length > 0 ? (
          ...
          <div className="text-center py-12 bg-white rounded-2xl border">
            ...
            <p className="text-gray-500 mb-4">Nenhum imóvel ativo no CONDOMINIO VILLAGGIO DI FIRENZI no momento.</p>
        ...
        {total > 12 && (
          <div className="text-center mt-6">
            <Link href={`/imoveis?city=Franca&neighborhood=CONDOMINIO VILLAGGIO DI FIRENZI`}
              ...
              Ver todos os {total} imóveis no CONDOMINIO VILLAGGIO DI FIRENZI <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        )}
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 pb-12">
        <div className="bg-white rounded-2xl p-8 border">
          <h2 className="text-xl font-bold mb-6" style={{ color: '#1B2B5B', fontFamily: 'Georgia, serif' }}>
            Perguntas Frequentes — Imóveis no CONDOMINIO VILLAGGIO DI FIRENZI
          </h2>
          <div className="space-y-5">
            <div>
              <h3 className="font-semibold text-gray-800 mb-1">Tem imóveis à venda no CONDOMINIO VILLAGGIO DI FIRENZI em Franca/SP?</h3>
              <p className="text-gray-600 text-sm">
                Sim! A Imobiliária Lemos tem {total > 0 ? `${total} imóveis` : 'imóveis'} disponíveis no CONDOMINIO VILLAGGIO DI FIRENZI, Franca/SP.
                Acesse nossa plataforma para ver casas, apartamentos e terrenos com fotos e preços completos.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-800 mb-1">Como alugar ou comprar no CONDOMINIO VILLAGGIO DI FIRENZI, Franca?</h3>
              <p className="text-gray-600 text-sm">
                ...
                Nossa equipe de corretores especializados atende o CONDOMINIO VILLAGGIO DI FIRENZI e toda Franca/SP.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-800 mb-1">Qual o preço dos imóveis no CONDOMINIO VILLAGGIO DI FIRENZI?</h3>
              <p className="text-gray-600 text-sm">
      ...
      <section className="py-10 px-4 text-center" style={{ background: 'linear-gradient(135deg, #1B2B5B, #0f1c3a)' }}>
        <div className="max-w-2xl mx-auto">
          ...
          <h2 className="text-xl font-bold text-white mb-2" style={{ fontFamily: 'Georgia, serif' }}>
            Imobiliária Lemos — CONDOMINIO VILLAGGIO DI FIRENZI, Franca/SP
          </h2>
