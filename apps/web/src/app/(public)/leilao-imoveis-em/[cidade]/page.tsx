...
export async function generateMetadata(props: { params: Promise<{ cidade: string }> }): Promise<Metadata> {
  ...
  if (!city) return { title: 'Imóveis em Leilão | AgoraEncontrei' }
  return {
    title: `Imóveis em Leilão em ${city.name}/${city.state} — Leilões de Imóveis | AgoraEncontrei`,
...
export default async function LeilaoCidadePage(props: { params: Promise<{ cidade: string }> }) {
  const params = await props.params
  const city = UNIQUE_CITIES.find(c => c.slug === params.cidade)
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'ItemList', name: `Imóveis em Leilão em ${city.name}/${city.state}`, url: `${WEB_URL}/imoveis-leilao-imoveis-em/${params.cidade}`, numberOfItems: properties.length }
        ...
        {nearby.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {nearby.map(c => (
                <Link key={c.slug} href={`/imoveis-leilao-imoveis-em/${c.slug}`} className="bg-white rounded-xl border p-3 text-center hover:border-[#C9A84C] transition text-sm">
