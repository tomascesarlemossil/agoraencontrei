...
export const metadata: Metadata = {
  title: 'Imóveis em São Paulo/SP — Venda, Aluguel e Leilões | AgoraEncontrei',
  description: 'Encontre imóveis em São Paulo/SP. Casas, apartamentos e terrenos à venda e para alugar. Leilões com até 50% de desconto. Marketplace imobiliário #1 do Brasil.',
  keywords: [
    'imóveis são paulo sp', 'casas à venda são paulo', 'apartamentos são paulo',
    'aluguel são paulo', 'leilão imóveis são paulo', 'terrenos são paulo',
    'imobiliária são paulo', 'comprar imóvel são paulo sp',
  ],
  openGraph: {
    title: 'Imóveis em São Paulo/SP | AgoraEncontrei',
    description: 'Marketplace imobiliário com imóveis à venda, aluguel e leilões em São Paulo/SP.',
    type: 'website', locale: 'pt_BR', siteName: 'AgoraEncontrei',
  },
  alternates: { canonical: 'https://www.agoraencontrei.com.br/imoveis-sao-paulo-sp' },
}

const BAIRROS = ['Pinheiros', 'Vila', 'Mariana', 'Moema', 'Itaim', 'Bibi', 'Brooklin', 'Perdizes', 'Santana', 'Tatuapé', 'Mooca', 'Campo', 'Belo', ]
...
export default async function CityPage() {
  ...
  const schema = {
    ...
    name: 'AgoraEncontrei — Imóveis em São Paulo/SP',
    url: 'https://www.agoraencontrei.com.br/imoveis-sao-paulo-sp',
    areaServed: { '@type': 'City', name: 'São Paulo', containedInPlace: { '@type': 'State', name: 'SP' } },
    description: 'Referência nacional em mercado imobiliário. Capital financeira do Brasil com maior volume de leilões de imóveis do país.',
  }
  ...
  return (
    <>
      ...
      <section className="bg-gradient-to-br from-[#1B2B5B] to-[#0f1c3a] text-white py-16 px-4">
        <div className="max-w-5xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 rounded-full px-4 py-1.5 text-xs font-semibold mb-4" style={{ color: '#C9A84C' }}>
            <MapPin className="w-3.5 h-3.5" /> São Paulo/SP · População 12.396.372
          </div>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4" style={{ fontFamily: 'Georgia, serif' }}>
            Imóveis em <span style={{ color: '#C9A84C' }}>São Paulo/SP</span>
          </h1>
          <p className="text-white/70 text-lg mb-6 max-w-2xl mx-auto">
            Referência nacional em mercado imobiliário. Capital financeira do Brasil com maior volume de leilões de imóveis do país.
          </p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href={`/imoveis?city=São Paulo`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm" style={{ background: '#C9A84C', color: '#1B2B5B' }}>
              <Search className="w-4 h-4" /> Buscar Imóveis
            </Link>
            <Link href={`/leiloes?city=São Paulo`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm bg-white/10 text-white border border-white/20">
              🏛️ Ver Leilões
            </Link>
            <Link href={`/custo-de-vida/sao-paulo-sp`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm bg-white/10 text-white border border-white/20">
              📊 Custo de Vida
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8 max-w-3xl mx-auto">
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <div className="text-xl font-bold" style={{ color: '#C9A84C' }}>R$ 9.500</div>
              <div className="text-xs text-white/60">m² médio</div>
            </div>
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <div className="text-xl font-bold" style={{ color: '#C9A84C' }}>R$ 3.200/mês</div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-10">
        {/* Bairros */}
        <section>
          <h2 className="text-xl font-bold text-gray-800 mb-4">Bairros em Destaque — São Paulo</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {BAIRROS.map((b: string) => (
              <Link key={b} href={`/imoveis?city=São Paulo&neighborhood=${encodeURIComponent(b)}`}
        ...
        {properties.length > 0 && (
          <section>
            <h2 className="text-xl font-bold text-gray-800 mb-4">Imóveis à Venda em São Paulo</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <section className="bg-gradient-to-r from-[#1B2B5B] to-[#2d4a8a] rounded-2xl p-8 text-center text-white">
          <h2 className="text-2xl font-bold mb-3" style={{ fontFamily: 'Georgia, serif' }}>
            Encontre seu imóvel ideal em São Paulo
          </h2>
          <p className="text-white/70 mb-6">Busca inteligente com IA. Leilões com desconto. Assessoria completa.</p>
          <a href="https://wa.me/5516981010004?text=Olá! Procuro imóvel em São Paulo/SP."
