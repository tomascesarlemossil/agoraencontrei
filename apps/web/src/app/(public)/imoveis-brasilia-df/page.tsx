...
export const metadata: Metadata = {
  title: 'Imóveis em Brasília/DF — Venda, Aluguel e Leilões | AgoraEncontrei',
  description: 'Encontre imóveis em Brasília/DF. Casas, apartamentos e terrenos à venda e para alugar. Leilões com até 50% de desconto. Marketplace imobiliário #1 do Brasil.',
  keywords: [
    'imóveis brasília df', 'casas à venda brasília', 'apartamentos brasília',
    'aluguel brasília', 'leilão imóveis brasília', 'terrenos brasília',
    'imobiliária brasília', 'comprar imóvel brasília df',
  ],
  openGraph: {
    title: 'Imóveis em Brasília/DF | AgoraEncontrei',
    description: 'Marketplace imobiliário com imóveis à venda, aluguel e leilões em Brasília/DF.',
    type: 'website', locale: 'pt_BR', siteName: 'AgoraEncontrei',
  },
  alternates: { canonical: 'https://www.agoraencontrei.com.br/imoveis-brasilia-df' },
}

const BAIRROS = ['Asa', 'Sul', 'Asa', 'Norte', 'Lago', 'Sul', 'Lago', 'Norte', 'Sudoeste', 'Noroeste', 'Águas', 'Claras', 'Guará', 'Taguatinga', 'Samambaia', ]
...
export default async function CityPage() {
  ...
  const schema = {
    ...
    name: 'AgoraEncontrei — Imóveis em Brasília/DF',
    url: 'https://www.agoraencontrei.com.br/imoveis-brasilia-df',
    areaServed: { '@type': 'City', name: 'Brasília', containedInPlace: { '@type': 'State', name: 'DF' } },
    description: 'Capital federal com renda per capita mais alta do Brasil. Mercado de alto padrão.',
  }
  ...
  return (
    <>
      ...
      <section className="bg-gradient-to-br from-[#1B2B5B] to-[#0f1c3a] text-white py-16 px-4">
        <div className="max-w-5xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 rounded-full px-4 py-1.5 text-xs font-semibold mb-4" style={{ color: '#C9A84C' }}>
            <MapPin className="w-3.5 h-3.5" /> Brasília/DF · População 3.094.325
          </div>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4" style={{ fontFamily: 'Georgia, serif' }}>
            Imóveis em <span style={{ color: '#C9A84C' }}>Brasília/DF</span>
          </h1>
          <p className="text-white/70 text-lg mb-6 max-w-2xl mx-auto">
            Capital federal com renda per capita mais alta do Brasil. Mercado de alto padrão.
          </p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href={`/imoveis?city=Brasília`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm" style={{ background: '#C9A84C', color: '#1B2B5B' }}>
              <Search className="w-4 h-4" /> Buscar Imóveis
            </Link>
            <Link href={`/leiloes?city=Brasília`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm bg-white/10 text-white border border-white/20">
              🏛️ Ver Leilões
            </Link>
            <Link href={`/custo-de-vida/brasilia-df`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm bg-white/10 text-white border border-white/20">
              📊 Custo de Vida
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8 max-w-3xl mx-auto">
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <div className="text-xl font-bold" style={{ color: '#C9A84C' }}>R$ 8.500</div>
              <div className="text-xs text-white/60">m² médio</div>
            </div>
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <div className="text-xl font-bold" style={{ color: '#C9A84C' }}>R$ 2.800/mês</div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-10">
        {/* Bairros */}
        <section>
          <h2 className="text-xl font-bold text-gray-800 mb-4">Bairros em Destaque — Brasília</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {BAIRROS.map((b: string) => (
              <Link key={b} href={`/imoveis?city=Brasília&neighborhood=${encodeURIComponent(b)}`}
        ...
        {properties.length > 0 && (
          <section>
            <h2 className="text-xl font-bold text-gray-800 mb-4">Imóveis à Venda em Brasília</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <section className="bg-gradient-to-r from-[#1B2B5B] to-[#2d4a8a] rounded-2xl p-8 text-center text-white">
          <h2 className="text-2xl font-bold mb-3" style={{ fontFamily: 'Georgia, serif' }}>
            Encontre seu imóvel ideal em Brasília
          </h2>
          <p className="text-white/70 mb-6">Busca inteligente com IA. Leilões com desconto. Assessoria completa.</p>
          <a href="https://wa.me/5516981010004?text=Olá! Procuro imóvel em Brasília/DF."
