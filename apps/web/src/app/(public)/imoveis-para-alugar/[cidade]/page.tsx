...
export async function generateMetadata(props: { params: Promise<{ cidade: string }> }): Promise<Metadata> {
  ...
  if (!city) return { title: 'Imóveis para Alugar | AgoraEncontrei' }
  return {
    title: `Imóveis para Alugar em ${city.name}/${city.state} — Casas e Apartamentos para Alugar | AgoraEncontrei`,
    description: `Encontre imóveis para alugar em ${city.name}/${city.state}. Casas, apartamentos, terrenos e leilões com até 50% de desconto. Marketplace imobiliário #1 do Brasil.`,
    keywords: [`imóveis para alugar ${city.name.toLowerCase()}`, `casas para alugar ${city.name.toLowerCase()} ${city.state.toLowerCase()}`, `apartamentos ${city.name.toLowerCase()}`, `terrenos ${city.name.toLowerCase()}`, `alugar imóvel ${city.name.toLowerCase()}`],
    openGraph: { title: `Imóveis para Alugar em ${city.name}/${city.state} | AgoraEncontrei`, description: `Casas, apartamentos e terrenos para alugar em ${city.name}.`, type: 'website', locale: 'pt_BR', siteName: 'AgoraEncontrei' },
    // Consolida com /{estado}/{cidade}/imoveis-para-alugar quando a cidade
    // existe no IBGE; senão mantém o auto-canonical.
    alternates: { canonical: canonicalFromCityUf(params.cidade, 'imoveis-para-alugar') ?? `${WEB_URL}/imoveis-para-alugar/${params.cidade}` },
  }
}
...
export default async function AluguelCidadePage(props: { params: Promise<{ cidade: string }> }) {
  const params = await props.params
  const city = UNIQUE_CITIES.find(c => c.slug === params.cidade)
  ...
  const schema = { '@context': 'https://schema.org', '@type': 'ItemList', name: `Imóveis para Alugar em ${city.name}/${city.state}`, url: `${WEB_URL}/imoveis-para-alugar/${params.cidade}`, numberOfItems: properties.length }
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-[#143A1F] to-[#0f1c3a] text-white py-14 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold mb-3" style={{ fontFamily: 'Georgia, serif' }}>Imóveis para Alugar em <span style={{ color: '#C9A84C' }}>{city.name}/{city.state}</span></h1>
          <p className="text-white/70 text-lg mb-6">{properties.length > 0 ? `${properties.length}+ imóveis disponíveis` : 'Imóveis disponíveis'} para aluguel em {city.name}.</p>
          <div className="flex flex-wrap gap-3">
            <Link href={`/imoveis?city=${city.name}&purpose=RENT`} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm" style={{ background: '#C9A84C', color: '#143A1F' }}><Search className="w-4 h-4" /> Buscar</Link>
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 space-y-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {PROPERTY_TYPES.map(t => (
            <Link key={t.slug} href={`/imoveis?city=${city.name}&type=${t.type}&purpose=RENT`} className="bg-white rounded-xl border p-3 text-center hover:border-[#C9A84C] transition text-sm font-medium text-gray-700">{t.label}</Link>
          ))}
        </div>
        ...
        {properties.length > 0 ? (
          ...
          <div className="text-center py-12 bg-white rounded-2xl border">
            ...
            <p className="text-gray-500 mb-4">Nenhum imóvel para alugar em {city.name} no momento.</p>
        ...
        {nearby.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {nearby.map(c => (
                <Link key={c.slug} href={`/imoveis-para-alugar/${c.slug}`} className="bg-white rounded-xl border p-3 text-center hover:border-[#C9A84C] transition text-sm">
