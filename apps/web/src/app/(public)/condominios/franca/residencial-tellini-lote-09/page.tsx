...
export const metadata: Metadata = {
  title: 'Imóveis no residencial TELLINI   Lote 09, Franca/SP | Imobiliária Lemos — CRECI 279051',
  description: 'Casas à venda e para alugar no residencial TELLINI   Lote 09 em Franca/SP. Imobiliária Lemos — 22 anos de tradição. CRECI 279051. Encontre o imóvel ideal no residencial TELLINI   Lote 09 com atendimento personalizado.',
  keywords: [
    'imóveis residencial tellini   lote 09 franca sp', 'casas residencial tellini   lote 09 franca', 'apartamentos residencial tellini   lote 09 franca',
    'comprar casa residencial tellini   lote 09 franca sp', 'alugar residencial tellini   lote 09 franca sp',
    'terrenos residencial tellini   lote 09 franca', 'imóveis franca sp residencial tellini   lote 09',
    'imobiliária lemos residencial tellini   lote 09', 'residencial tellini   lote 09 franca sp imóveis',
    'casas à venda residencial tellini   lote 09', 'apartamentos aluguel residencial tellini   lote 09',
  ],
  openGraph: {
    title: 'Imóveis no residencial TELLINI   Lote 09, Franca/SP | Imobiliária Lemos',
    description: 'Encontre casas, apartamentos e terrenos no residencial TELLINI   Lote 09 em Franca/SP com a Imobiliária Lemos.',
    type: 'website', locale: 'pt_BR', siteName: 'AgoraEncontrei — Imobiliária Lemos',
  },
  alternates: { canonical: 'https://www.agoraencontrei.com.br/bairros/franca/residencial-tellini-lote-09' },
...
const SCHEMA = {
  ...
  name: 'Imóveis no residencial TELLINI   Lote 09, Franca/SP',
  description: 'Imóveis disponíveis no residencial TELLINI   Lote 09 em Franca, São Paulo. Imobiliária Lemos — CRECI 279051.',
  url: 'https://www.agoraencontrei.com.br/bairros/franca/residencial-tellini-lote-09',
  provider: {
...
async function fetchProperties() {
  try {
    const res = await fetch(
      `${API_URL}/api/v1/public/properties?city=Franca&search=residencial TELLINI   Lote 09=${encodeURIComponent('residencial TELLINI   Lote 09')}&limit=12&status=ACTIVE&sortBy=createdAt&sortOrder=desc`,
      { next: { revalidate: 600 } }
    )
      ...
      <section className="bg-gradient-to-br from-[#1B2B5B] to-[#0f1c3a] text-white py-14 px-4">
        <div className="max-w-5xl mx-auto">
          <nav className="text-xs text-blue-200 mb-4 flex items-center gap-1.5 flex-wrap">
            ...
            <span>/</span>
            <span className="text-white">residencial TELLINI   Lote 09</span>
          </nav>
          <h1 className="text-3xl sm:text-4xl font-bold mb-3" style={{ fontFamily: 'Georgia, serif' }}>
            Imóveis no <span style={{ color: '#C9A84C' }}>residencial TELLINI   Lote 09</span>
            <br /><span className="text-xl font-normal text-white/70">Franca/SP</span>
          </h1>
          <p className="text-white/70 text-base mb-5 max-w-2xl">
            {total > 0 ? `${total} imóveis disponíveis` : 'Imóveis disponíveis'} no residencial TELLINI   Lote 09, Franca/SP.
            Casas, apartamentos, terrenos e comerciais. Imobiliária Lemos — CRECI 279051.
          </p>
          <div className="flex flex-wrap gap-2">
            <Link href={`/imoveis?city=Franca&neighborhood=residencial TELLINI   Lote 09&type=HOUSE`}
            ...
            <Link href={`/imoveis?city=Franca&neighborhood=residencial TELLINI   Lote 09&type=APARTMENT`}
              className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-full text-sm font-medium border border-white/20 transition-colors">
              Apartamentos
            </Link>
            <Link href={`/imoveis?city=Franca&neighborhood=residencial TELLINI   Lote 09&type=LAND`}
              className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-full text-sm font-medium border border-white/20 transition-colors">
              Terrenos
            </Link>
            <Link href={`/imoveis?city=Franca&neighborhood=residencial TELLINI   Lote 09&purpose=RENT`}
              className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-full text-sm font-medium border border-white/20 transition-colors">
              Para Alugar
            </Link>
            <a href="https://wa.me/5516981010004?text=Olá! Quero um imóvel no residencial TELLINI   Lote 09 em Franca/SP."
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold" style={{ color: '#1B2B5B', fontFamily: 'Georgia, serif' }}>
            {total > 0 ? `${total} Imóveis no residencial TELLINI   Lote 09` : 'Imóveis no residencial TELLINI   Lote 09'}
          </h2>
          <Link href={`/imoveis?city=Franca&neighborhood=residencial TELLINI   Lote 09`}
        ...
        {properties.length > 0 ? (
          ...
          <div className="text-center py-12 bg-white rounded-2xl border">
            ...
            <p className="text-gray-500 mb-4">Nenhum imóvel ativo no residencial TELLINI   Lote 09 no momento.</p>
        ...
        {total > 12 && (
          <div className="text-center mt-6">
            <Link href={`/imoveis?city=Franca&neighborhood=residencial TELLINI   Lote 09`}
              ...
              Ver todos os {total} imóveis no residencial TELLINI   Lote 09 <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        )}
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 pb-12">
        <div className="bg-white rounded-2xl p-8 border">
          <h2 className="text-xl font-bold mb-6" style={{ color: '#1B2B5B', fontFamily: 'Georgia, serif' }}>
            Perguntas Frequentes — Imóveis no residencial TELLINI   Lote 09
          </h2>
          <div className="space-y-5">
            <div>
              <h3 className="font-semibold text-gray-800 mb-1">Tem imóveis à venda no residencial TELLINI   Lote 09 em Franca/SP?</h3>
              <p className="text-gray-600 text-sm">
                Sim! A Imobiliária Lemos tem {total > 0 ? `${total} imóveis` : 'imóveis'} disponíveis no residencial TELLINI   Lote 09, Franca/SP.
                Acesse nossa plataforma para ver casas, apartamentos e terrenos com fotos e preços completos.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-800 mb-1">Como alugar ou comprar no residencial TELLINI   Lote 09, Franca?</h3>
              <p className="text-gray-600 text-sm">
                ...
                Nossa equipe de corretores especializados atende o residencial TELLINI   Lote 09 e toda Franca/SP.
              </p>
            </div>
            <div>
              <h3 className="font-semibold text-gray-800 mb-1">Qual o preço dos imóveis no residencial TELLINI   Lote 09?</h3>
              <p className="text-gray-600 text-sm">
      ...
      <section className="py-10 px-4 text-center" style={{ background: 'linear-gradient(135deg, #1B2B5B, #0f1c3a)' }}>
        <div className="max-w-2xl mx-auto">
          ...
          <h2 className="text-xl font-bold text-white mb-2" style={{ fontFamily: 'Georgia, serif' }}>
            Imobiliária Lemos — residencial TELLINI   Lote 09, Franca/SP
          </h2>
